function hFig = DOTHUB_LUMOplotArray(y,tHRF,SD,yLimits,distRange,overSizeRatios,titString,fontSize,lineWidth)
    ...
    mnVal = min(tmpy(:));
    mxVal = max(tmpy(:));
    fprintf(['Maximum and minimum data values = [' num2str(mnVal,'%3f') ' ' num2str(mxVal,'%3f') ']  �M\n']);
    ...
    dfX = diff(xPosSort);
    shift = dfX(dfX>gapThresh);
    if ~isempty(shift)
        xOccupied = range(xPosSort) - max(shift);
    else
        xOccupied = range(xPosSort);
    end
    
    yPosSort = sort(unique(posCropRound(:,2)));
    dfY = diff(yPosSort);
    shift = dfY(dfY>gapThresh);
    if ~isempty(shift)
        yOccupied = range(yPosSort) - max(shift);
    else
        yOccupied = range(yPosSort);
    end
